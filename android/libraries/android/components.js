...
prx.types.android_statusbar = {
	...
	,onDisplay: function(item,containerid,pageid,symbol) {
		...
		cR += '<style>';
		cR += '#'+_id+' .android-statusbar-inner { font-size: '+(item.height/2)+'px; background-color: '+getColor(item.backgroundColor)+'; color: '+getColor(item.foregroundColor)+'; }';
		cR += '#'+_id+' .android-statusbar-signal-line.android-statusbar-signal-line-active { background-color: '+getColor(item.foregroundColor)+'; }';
		cR += '#'+_id+' .android-statusbar-battery-life-icon { background-color: '+getColor(item.foregroundColor)+'; border-color: '+getColor(item.foregroundColor)+'; width: '+item.height+'px; }';
		cR += '#'+_id+' .android-statusbar-battery-life-icon-battery-pole-inner { background-color: '+getColor(item.foregroundColor)+'; }';
		cR += '</style>';
		...
		cR += '</div>';
		cR += '</div>';
		cR += '<div class="android-statusbar-time">16:20</div>';
		cR += '</div>';
		cR += '</div>';
		return cR;
	}
...
prx.types.android_navigationbar = {
	...
	,onDisplay: function(item,containerid,pageid,symbol) {
		...
		cR += '<style>';
		cR += '#'+_id+' .android-navigationbar-inner { background-color: '+getColor(item.backgroundColor)+'; }';
		cR += '</style>';
		
		cR += '<div class="android-navigationbar-inner liveUpdate-backgroundColor">';
		cR += '<div class="android-navigationbar-button" style="background-image: url('+getAssetUrl(item.icon1)+')"></div>';
		cR += '<div class="android-navigationbar-button" style="background-image: url('+getAssetUrl(item.icon2)+')"></div>';
		cR += '<div class="android-navigationbar-button" style="background-image: url('+getAssetUrl(item.icon3)+')"></div>';
		cR += '</div>';
		cR += '</div>';
		return cR;
	}
